...
def main(args):
    ...
    try:
        ...
        if args.data_set == 'IMNET':
            if args.data_path == '/root/FastBaseline/data/imagenet' or not os.path.exists(args.data_path):
                args.data_path = kaggle_config.IMAGENET_PATH
        elif args.data_set == 'CIFAR':
            if not os.path.exists(args.data_path):
                args.data_path = kaggle_config.CIFAR_PATH
                
        if args.output_dir == 'checkpoints':
            args.output_dir = kaggle_config.CHECKPOINT_DIR
            
        if not args.inc_path:
            args.inc_path = kaggle_config.get_dataset_path("imagenet-c", ["imagenet-c", "corruption"])
        if not args.ina_path:
            args.ina_path = kaggle_config.get_dataset_path("imagenet-a", ["imagenet-a", "adversarial"])
        if not args.inr_path:
            args.inr_path = kaggle_config.get_dataset_path("imagenet-r", ["imagenet-r", "rendition"])
        if not args.insk_path:
            args.insk_path = kaggle_config.get_dataset_path("imagenet-sketch", ["imagenet-sketch", "sketch"])
    except ImportError:
        pass
    ...
    # Enable TF32 on Turing (T4) and later — near-FP32 accuracy with tensor-core throughput
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and ((epoch + 1) % args.save_freq == 0 or epoch == args.epochs - 1):
            ...
            remove_epoch = epoch - 3 * args.save_freq
            if remove_epoch >= 0 and utils.is_main_process():
                old_ckpt = os.path.join(output_dir, 'checkpoint_'+str(remove_epoch)+'.pth')
