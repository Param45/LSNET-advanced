...
# Clean default_cfgs for compatibility with newer timm versions at import time
import inspect
try:
    from timm.models._registry import PretrainedCfg
    sig = inspect.signature(PretrainedCfg.__init__)
    params = sig.parameters
    if 'hf_hub' not in params:
        for k, cfg in default_cfgs.items():
            if 'hf_hub' in cfg:
                if 'hf_hub_id' in params:
                    cfg['hf_hub_id'] = cfg.pop('hf_hub')
                else:
                    cfg.pop('hf_hub', None)
except Exception:
    pass
...
def _create_lsnet(variant, pretrained=False, **kwargs):
    cfg = default_cfgs[variant].copy()
    
    # Adapt default_cfg dict to avoid unexpected keyword arguments in PretrainedCfg
    import inspect
    try:
        from timm.models._registry import PretrainedCfg
        sig = inspect.signature(PretrainedCfg.__init__)
        params = sig.parameters
        if 'hf_hub' not in params:
            if 'hf_hub_id' in params and 'hf_hub' in cfg:
                cfg['hf_hub_id'] = cfg.pop('hf_hub')
            else:
                cfg.pop('hf_hub', None)
    except Exception:
